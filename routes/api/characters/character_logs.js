...
router.get('/:nameSlug@:realmSlug', async function(req, res) {
    try {
        ...
        if (nameSlug && realmSlug) {
            let characterLogs = await osint_logs.find({type: "character", root_id:`${nameSlug}@${realmSlug}`}).sort({createdAt: -1}).lean();
            if (!characterLogs.length) {
                let realm = await realms_db.findOne({ $text: { $search: realmSlug } });
                if (realm) {
                    characterLogs = await osint_logs.find({type: "character", root_id:`${nameSlug}@${realmSlug}`}).sort({createdAt: -1}).lean();
                } else {
