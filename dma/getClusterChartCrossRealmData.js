...
async function auctionsCrossRealmData (item_id = 168487) {
    try {
        ...
        for (let order of orders) {
            ...
            /** yAxis */
            const corrected_price = priceRange_array.reduce((prev, curr) => Math.abs(curr - order._id.price) < Math.abs(prev - order._id.price) ? curr : prev)
        ...
        return { price_range: priceRange_array, realms: realms, dataset: chartArray }
    } catch (error) {
        console.error(error)
    }
}
