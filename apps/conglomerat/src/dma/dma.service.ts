...
import {
  ...
  WowtokenDto,
  ItemValuationsDto,
  ReqGetItemDto, IBuildYAxis, IGetCommdtyOrders,
} from '@app/core';
import { InjectModel } from '@nestjs/mongoose';
import { Auction, Gold, Item, Realm, Token, Valuations } from '@app/mongo';
import { Aggregate, FilterQuery, LeanDocument, Model } from 'mongoose';
...
export class DmaService {
  ...
  async getGoldChart(input: ItemCrossRealmDto): Promise<ItemChartDto> {
    ...
    const yAxis = await this.buildYAxis({ itemId: item._id, connectedRealmsIds, isCommdty, isXrs, isGold });
  ...
  async buildYAxis(args: IBuildYAxis): Promise<number[]> {
    ...
    if (isGold) {
      ...
      if (connectedRealmsIds) {
        Object.assign(searchByGold, { connected_realm_id: { $in: connectedRealmsIds } });
      }

      if (isXrs) {
        const { golds } = await this.RealmModel.findOne({ connected_realm_id: { $in: connectedRealmsIds } }).lean().select('golds').sort({ 'golds': 1 });
        Object.assign(searchByGold, { last_modified: { $gte: golds } });
      }

      const quotes: number[] = await this.GoldModel.find(searchByGold, 'price').distinct('price');
      return this.priceRangeY(quotes, blocks);
    }
    ...
    if (!isGold && !isCommdty) {
      ...
      if (connectedRealmsIds) {
        Object.assign(searchByAuctions, { connected_realm_id: { $in: connectedRealmsIds } });
      }

      if (isXrs) {
        const { auctions } = await this.RealmModel.findOne({ connected_realm_id: { $in: connectedRealmsIds } }).lean().select('auctions').sort({ 'auctions': 1 });
  ...
  async getOrderQuotes(args: IGetCommdtyOrders): Promise<Aggregate<Array<IOrderQuotes>>> {
