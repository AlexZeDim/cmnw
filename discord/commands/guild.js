...
module.exports = {
    name: 'guild',
    description: 'Return information about specific guild. Example usage: \`guild депортация@гордунни\`',
    aliases: ['GUILD', 'Guild'],
    args: true,
    async execute(message, args) {
        const [name, realm] = args.split('@');
        let embed = new MessageEmbed();
        let guild = await axios.get(encodeURI(`http://${process.env.localhost}:3030/api/guilds/guild/${name}@${realm}`)).then(({data}) => {
            let {
                _id,
                name,
                realm,
            ...
            const fieldsToCheck = ['id', 'achievement_points', 'faction', 'member_count', 'lastModified', 'createdBy']
