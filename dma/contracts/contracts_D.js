    ...
    for (let { _id, ticker, name } of items) {
      ...
      for (let connected_realm_id of realms) {
        ...
        if (contract_data && contract_data.length) {
          ...
          contract_data.map(
            (
              ...
              if (orders) {
                ...
                let orders_T0 = orders_Array.filter(
                  (val) => !expired_Array.includes(val)
                );
          ...
          contract.risk = {
            stdDev: Round2(
              standardDeviation(price_Array.map((price) => price * 100)) / 100
            ),
          };
          ...
          if (price_size_Array && price_size_Array.length) {
            contract.risk.stdDev_size = Round2(
              standardDeviation(
                price_size_Array.map((price_size) => price_size * 100)
              ) / 100
            );
            contract.price_size = {
          ...
          if (orders_count_Array && orders_count_Array.length) {
            contract.orders = {
              ...
              change:
                orders_count_Array[orders_count_Array.length - 1] -
                orders_count_Array[0],
              ...
              ratio_cancelled: Round2(
                cancelled_orders.size / total_orders.size
              ),
          ...
          contract.quantity = {
            ...
            change:
              quantity_Array[quantity_Array.length - 1] - quantity_Array[0],
          ...
          contract.open_interest = {
            ...
            change: parseFloat(
              (
                open_interest_Array[open_interest_Array.length - 1] -
                open_interest_Array[0]
              ).toFixed(2)
            ),
          ...
          if (sellersArray.size !== 0) {
            contract.sellers = {
              ...
              change:
                contract_data[contract_data.length - 1].sellers.length -
                contract_data[0].sellers.length,
          ...
          console.error(
            `E,${code}-${moment().format("DD.MMM")}@${connected_realm_id}`
          );
        }
      }
    }
