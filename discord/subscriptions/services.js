    ...
    switch (type) {
      case 'recruiting':
        ...
        await Promise.all(characters.map(async character => {
          ...
          if (character.lfg) {
            if (character.lfg.role) embed.addField('Role', character.lfg.role.map(r => `${r.toString().toUpperCase()}\n`), true);
...
/**
 * @param _id {string}
 * @param fault_tolerance {number}
 * @param discord_name {string=}
 * @param channel_name {string=}
 * @returns {Promise<void>}
 */
const errorFaultTolerance = async ({ _id, fault_tolerance, discord_name, channel_name }) => {
  try {
    await discord_db.findByIdAndUpdate(_id, { fault_tolerance: (fault_tolerance || 0) + 1})
    if (fault_tolerance && fault_tolerance > 9) await discord_db.deleteOne({ _id: _id })
    console.warn(`DISCORD: ${discord_name}@${channel_name} unable to fetch`)
  } catch (error) {
    console.error(`E,${errorFaultTolerance.name}:${error}`)
  }
}

module.exports = { errorFaultTolerance, worker, installer };
