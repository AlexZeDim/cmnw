    ...
    try {
        ...
        if (guild.statusCode === 200) {
            ...
            await bnw.WowProfileData.getGuildRoster(realmSlug, nameSlug).then(async ({members}) => {
                ...
                for (let member of members) {
                    ...
                    if (character && rank) {
                        ...
                        let character_OSINT = await characters_db.findById(`${character.name}@${guild.realm_slug}`);
                        ...
                        if (character_OSINT) {
                            ...
                            let character_added = await getCharacter(realmSlug, character.name, character_Object, token, updatedBy)
            ...
            if (guild.members.length && guild_created.members.length) {
                 ...
                 */
                let leave = guild.members.filter(({ character_id: id1 }) => !guild_created.members.some(({ character_id: id2 }) => id2 === id1));
                if (leave.length) {
                    ...
                    await updateCharactersGuildHistory(leave, guild.id, guild.name, guild.realm_slug, guild.lastModified, guild_created.lastModified, 'leaves');
                }
                /**
                 * If in old_members character_id rank was lower then in latest_members then you have been promoted
                 */
                let promote = guild_created.members.filter(({ character_id: id1, character_rank: r1 }) => guild.members.some(({ character_id: id2, character_rank: r2 }) => id2 === id1 && r2 > r1));
                if (promote.length) {
                    ...
                    await updateCharactersGuildHistory(promote, guild.id, guild.name, guild.realm_slug, guild.lastModified, guild_created.lastModified, 'promoted');
                }
                /**
                 * If in latest_members character_id rank was lower then in old_members then you have been demoted
                 */
                let demote = guild.members.filter(({ character_id: id1, character_rank: r1 }) => guild_created.members.some(({ character_id: id2, character_rank: r2 }) => id2 === id1 && r2 > r1));
                if (demote.length) {
                    ...
                    await updateCharactersGuildHistory(demote, guild.id, guild.name, guild.realm_slug, guild.lastModified, guild_created.lastModified, 'demoted');
                }
                /**
                 * If latest_members have character_id rank and old_members don't have the same id, then it's a newcomer
                 */
                let join = guild_created.members.filter(({character_id: id1}) => !guild.members.some(({character_id: id2}) => id2 === id1));
                if (join.length) {
                    console.info(`JOIN: ${join.length} => ${guild_created.guild_log.join.length}`);
                    guild.guild_log.join = [...guild.guild_log.join, ...join];
                    await updateCharactersGuildHistory(join, guild.id, guild.name, guild.realm_slug, guild.lastModified, guild_created.lastModified, 'joins');
                }
                /**
                 * TODO Transfer ownership
                 *
                 * let ownership_old = guild_old.members.find(({character_rank}) => character_rank === 0);
                 * let ownership_new = guild.members.find(({character_rank}) => character_rank === 0);
