...
router.get('/:itemName@:realmSlug', async (req, res) => {
    try {
        ...
        if (item && realm) {
            let contracts = await contracts_db.find({item_id: item._id, connected_realm_id: realm.connected_realm_id, "date.month": moment().subtract(1,'month').get('month')+1}).lean();
