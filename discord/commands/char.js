...
module.exports = {
    ...
    async execute(message, args) {
        ...
        let character = await axios.get(encodeURI(`http://${process.env.localhost}:3030/api/characters/character/${name}@${realm}`)).then(({data}) => {
            ...
            const fieldsToCheck = ['id', 'character_class', 'spec', 'level', 'hash', 'ilvl', 'faction', 'media', 'race', 'gender', 'lastModified', 'createdBy']
                ...
                if (field in data) {
                    if (typeof data[field] === 'object') {
                        ...
                        if (field === 'faction') {
                            ...
                            embed.addField(humanizeString(field.replace('character_', '')), data[field], true);
                        }
