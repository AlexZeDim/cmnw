...
const ClusterChartCrossRealmData = require('../../../dma/valuations/cluster/item_xrs.js');
...
router.get('/:itemQuery', async function (req, res) {
  try {
    ...
    if (item) {
      ...
      if (is_commdty) {
        await ClusterChartCrossRealmData(item._id).then(chart =>
          Object.assign(response, { chart: chart }),
        );
      } else {
        await auctionsFeed(item._id).then(feed =>
