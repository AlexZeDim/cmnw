            ...
            try {
                ...
                if (wcl_log && wcl_log.exportedCharacters && wcl_log.exportedCharacters.length) {
                    for (let character of wcl_log.exportedCharacters) {
                        ...
                        if (realm && realm.slug) {
                            let character_OSINT = await characters_db.findById(toSlug(`${character.name}@${realm.slug}`));
                            if (!character_OSINT) {
                                await getCharacter(realm.slug, character.name, {}, token, `OSINT-${indexLogs.name}`)
                            }
                        }
                    }
                }
