...
async function getClusterChartData (item_id = 152510, connected_realm_id = 1602) {
    try {
        ...
        if (item_id === 1) {
            ...
            price_query = { createdAt: {$gt: ytd}, connected_realm_id: connected_realm_id }
            price_field = 'price'
            Model = golds_db;
        } else {
            price_query = { "item.id": item_id, connected_realm_id: connected_realm_id }
        ...
        if (quotes.length && timestamp.length) {
            ...
            for (let order of orders) {
                ...
                /** yAxis */
                const corrected_price = priceRange_array.reduce((prev, curr) => Math.abs(curr - order[price_field]) < Math.abs(prev - order[price_field]) ? curr : prev)
