...
export class AuctionsWorker {
  private readonly logger = new Logger(AuctionsWorker.name, { timestamp: true });
  ...
  public async process(job: Job<IQAuction, number>): Promise<number> {
    try {
      ...
      let ifModifiedSince = `${moment(0)
        .utc()
        .format('ddd, DD MMM YYYY HH:mm:ss')} GMT`;
      let getAuctionsMethodApi = '/data/wow/auctions/commodities';
      ...
      if (!args.connected_realm_id) {
        ...
        if (isModifiedSinceExists) {
          ...
          ifModifiedSince = `${moment(ifModifiedSinceTimestamp)
            .utc()
            .format('ddd, DD MMM YYYY HH:mm:ss')} GMT`;
        }
      } else {
        ...
        if (!args.auctions || args.auctions === 0) {
          ...
          ifModifiedSince = `${moment(args.auctions)
            .utc()
            .format('ddd, DD MMM YYYY HH:mm:ss')} GMT`;
        }
      ...
      const auctionsResponse: IAuctionsResponse = await this.BNet.query(
        getAuctionsMethodApi,
        {
      ...
      if (
        !auctionsResponse ||
        !Array.isArray(auctionsResponse.auctions) ||
        !auctionsResponse.auctions.length
      )
        return 504;

      const ts: number = parseInt(moment(auctionsResponse.lastModified).format('x'));

      const orders = await this.writeBulkOrders(
      ...
      if (args.connected_realm_id) {
        await job.log(
          `${AuctionsWorker.name}: ${args.connected_realm_id}:${orders}`,
        );
      ...
      await lastValueFrom(
        from(orders).pipe(
          ...
          concatMap(async (ordersBulk) => {
            ...
            await this.AuctionModel.insertMany(ordersBulkAuctions, {
              rawResult: false,
              lean: true,
            });
