...
module.exports = {
  name: 'contracts',
  description:
    'Returns a min, avg, max price and quantity for the certain item on requested tenor. Example usage: `contracts tod zntd@gordunni` ',
  ...
  async execute(message, args) {
    ...
    if (checkTenor.includes(tenor) && query) {
      ...
      let contracts = await axios
        .get(
          encodeURI(
            `http://${process.env.localhost}:3030/api/contracts/${tenor}/${name}@${realm}`,
          ),
        )
        .then(({ data }) => {
          ...
          if (contracts.length) {
            embed.setDescription(
              `Item data is based on ${contracts.length} contracts`,
            );
          }
